...
- **Root cause of a real 90-minute live-capture-into-transcribe crash**: confirmed via real `journalctl`/`free -h` output that the droplet's local Whisper transcription OOM-killed under a genuine 90-min live job — the droplet only has ~1.9GB RAM. Fix: transcription now routes to Deepgram's managed API when `DEEPGRAM_API_KEY` is set (`tools/basiq_agent.py`'s `transcribe_with_deepgram`), with the original local-Whisper path kept as the fallback when the key is absent. Batch (non-live) transcription and live-clip transcription can run on different cost tiers by design — same code path, gated purely on whether the job is live.
- **New: a real-time transcript feed during live capture itself** (`stream_live_transcript_to_deepgram`/`build_audio_tee_cmd`) — a second, audio-only ffmpeg process taps the same live `stream_url` independently of the primary recording and streams PCM to Deepgram's live API. Verified end-to-end against a synthetic live source served over real local HTTP (matching production's actual network-stream semantics, not a bare file path — an earlier local-file-path test produced a false-negative hang that doesn't reflect how production actually reads a stream). Currently only stored in-memory on the job status (`live_transcript` field) — no persistence or UI yet, that's unscoped follow-on work.
- Fixed a real display bug: live captures showed KIND "Download" (no `is_live` column existed; `source_kind` didn't distinguish live from downloaded). Backfilled via migration `0013_live_capture_source_kind.sql`.
- Closed an RLS security gap Supabase flagged by email on 4 public tables (enabled RLS + public-read policies).
- Shipped: universal COPY PATH via the team's shared LucidLink drive (no longer requires the local agent), and a minimal `/m` mobile route for grabbing a link from a phone.
- **Deploy notes**: this fast-forwarded the droplet from several sessions behind (`c966660` → `fbc49d2`) — all of the above shipped together in one push+restart. Also found and fixed two real gaps during this deploy: `websocket-client` (new dependency for the live-transcript feed) wasn't installed in the droplet's venv, and `DEEPGRAM_API_KEY` — despite an earlier belief it had been added — was not actually present in `/etc/basiq-agent.env`. Both fixed; `basiq-agent` restarted and confirmed healthy (`/health` on port 8000, not 8787).
- **Still needs a real, user-triggered live capture test** to confirm the Deepgram paths end-to-end in production — not run automatically, per standing policy.

### 2026-09-14 later — GO LIVE on YouTube broke after GRAB was fixed; found a second real gap and reworked the proxy trigger to self-adapt instead of hardcoding sites

Right after confirming a real GRAB worked (see below), a real GO LIVE attempt on a YouTube live stream failed with an ffmpeg error (`Invalid data found when processing input`), and a real CBS Live attempt failed with `[cbsnews:live] ... HTTP Error 406`. Neither was a repeat of the earlier deploy/staleness mistakes — both root-caused properly:

1. **YouTube live capture never actually used the proxy at all.** `YTDLP_PROXY` only ever fed into `base_opts()`, which resolves the manifest URL (via yt-dlp) but isn't what fetches the actual video — `run_live_capture()` hands the resolved manifest URL straight to **ffmpeg**, a separate subprocess that makes its own HTTP connections and never saw the proxy setting. The failing manifest URL itself proved it: it had `ip/48.46.212.179` (one of the proxy IPs, from resolving the manifest) *and* `mip/137.184.99.201` (the droplet's real IP) both baked into the same signed URL — ffmpeg was fetching segments from the droplet's own IP against a manifest signed for a different IP entirely, and YouTube's CDN rejected it. Fixed: `resolve_live_stream()` now returns whichever proxy it used, and `run_live_capture()` hands that *exact same* value to ffmpeg via a new `-http_proxy` flag in `build_capture_cmd()` — resolving and fetching now always agree on which IP they're using.
2. **CBS's real error was being hidden.** `run_live_capture()` tries yt-dlp's own extractor first, falls back to a generic headless-browser resolver (`resolve_live_stream_generic()`) for sites yt-dlp doesn't cover, and — if *that* also failed — re-raised the *original* yt-dlp exception instead of the generic resolver's own. Since CBS has always gone through the generic path, the "406" shown was never the real current failure, just whichever error happened to fire first; the generic resolver's actual (and more relevant) exception was silently discarded every time. Fixed to combine both messages so the next real failure shows what's actually broken.

**Also reworked, from direct user pushback:** the first pass at this whole fix scoped `YTDLP_PROXY` to a hardcoded tuple of YouTube hostnames. Correctly called out as the wrong shape — "i don't want to have to pre-load every possible URL i'm ever going to need to grab from." Reworked so **no site list exists at all**: a grab's (and now a live capture's) first attempt always goes out with no proxy, and only a *retry* — meaning the direct attempt already failed with a transient/blocking-shaped error, per the existing `_retryable()` classifier — adds one. Self-adapts to whichever site actually needs a clean IP, with no code change required when some other platform starts (or stops) needing it. See `tools/basiq_agent.py`'s `base_opts(referer, use_proxy)` and `resolve_live_stream()`.

**Verified, both real, human-initiated:** a YouTube GO LIVE succeeded. A CBS Live retry then failed differently — `playwright is not installed` — confirmed the droplet's venv had never actually had playwright installed at all (listed in `requirements.txt`, but the venv predated that line and was never resynced; same root class as the yt-dlp staleness above, different package). Fixed by installing playwright+chromium on the droplet and broadening the weekly dependency timer to run the full `pip install -r requirements.txt` (plus yt-dlp/playwright specifically), not just the yt-dlp bump, so a future added dependency doesn't silently go missing the same way. **CBS Live confirmed working after that fix**, real human-initiated retry.

Also surfaced, same testing session: **Bloomberg Live fails, and it's a genuinely different, harder problem** — its player loads video via `blob:` URLs (MediaSource Extensions), so no plain `.m3u8`/`.mpd` manifest URL ever appears in observable network traffic at all, which is what the generic resolver (`resolve_live_stream_generic()`) depends on finding. yt-dlp's own dedicated Bloomberg extractor also fails on this specific `/live/us` URL shape. **Confirmed with the user: this was a one-off spot-check, not an actual need — deliberately not built out.** Leave as-is unless that changes.
...
**Update, same day, later:** signed up for Decodo's 3-IP Dedicated ISP plan ($9.99/mo, unlimited bandwidth, US-geo-targeted — real fair-use cap is 100GB/IP/month, ~300GB/month total, plenty for current volume). All 3 IPs verified working individually (real distinct US residential IPs, confirmed via a neutral IP-lookup service, never YouTube) before going anywhere near production. `base_opts()` now rotates across all 3 (comma-separated `YTDLP_PROXY`) so no single IP carries all the traffic.

**Two real deploy mistakes hit and fixed while turning this on, both worth remembering:**
1. **Set the env vars and restarted the droplet's `basiq-agent`, but never actually deployed the code that reads them** — `YTDLP_PROXY`/`COOKIES_FILE` support only existed on the local `feat/saas-cloud-proxy-grab` branch, which had never been pushed to GitHub, so the droplet kept running old `master` code with zero proxy logic. Two real, human-initiated GRAB attempts failed identically as a result, both looking like a proxy/cookie problem when the real issue was that none of the fix had actually reached production. Confirmed and fixed by explicitly diffing the droplet's running `git log -1` + `grep`-ing the deployed file for the new symbol, not just trusting "the service restarted without error." **Standing practice going forward: state explicitly, every time, which of local files / git push / droplet SSH+restart / third-party dashboard a change has actually reached, and verify the target directly — don't assume a clean restart means the fix is live.**
2. **After the code was genuinely deployed** (with cookies also copied over, `check_cookies.py`-validated first), grabs still failed with the identical "Sign in to confirm you're not a bot" error. Root cause: the droplet's yt-dlp was `2026.7.4` — over a month stale versus the `2026.8.30.232658.dev0` nightly the Windows worker has run since 9/11 specifically to fight evolving YouTube bot-detection. `requirements.txt` only pins `yt-dlp>=2024.1.0`, so a normal `pip install -r requirements.txt` never upgrades it once that floor is met — the droplet's copy simply never got the manual bump the worker got, since it never used to run real grabs at all. The PO-token service (`bgutil`) confirmed never even contacted during any of the failed attempts, consistent with the block happening earlier than that, at the extractor level. Fixed with `pip install --pre --upgrade yt-dlp` on the droplet to match the same nightly, restarted. **Worth remembering: the droplet's yt-dlp can silently drift stale exactly because it always "successfully" satisfies the unpinned floor — it needs the same proactive nightly-bump treatment the worker gets, now that it actually runs grabs.**

**Not yet confirmed working — still needs the user:**
1. One more real, human-initiated GRAB from the actual web UI, now that both the deploy gap and the stale yt-dlp are fixed — not an automated test call, per the standing YouTube-testing rule.
2. Only once that holds up for a few days: consider retiring the Windows worker/tray/pipeline-doctor/LucidLink-service stack for GRAB specifically (park it, don't delete it — it's still needed if the proxy ever needs a fallback).
