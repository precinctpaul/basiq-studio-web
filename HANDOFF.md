...
7. **Small repair-pass backlog, bounded, not urgent:** ~7 transcription candidates + 2 Item-3 candidates (`cspan_662803.mp4`, `cspan_667238.mp4`) have DB rows whose file doesn't exist on disk at all (need re-linking or removal), and ~15 more transcription candidates have a suspiciously near-zero `duration_seconds` (a probe-failure artifact, needs re-probing) — both excluded from their respective batches above rather than counted as real failures.
